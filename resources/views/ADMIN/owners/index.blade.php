            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                                        ...
                                        @foreach ($owners as $owner)
                                            <tr>
                                                ...
                                                <td class="px-4 py-3">
                                                    ...
                                                    class="text-white bg-red-400 border-0 py-2 px-4 focus:outline-none hover:bg-red-500 rounded">編集</button>
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                    ...
                    @foreach ($q_get as $q_owner)
                        {{ $q_owner->name }}
                        {{ Carbon\Carbon::parse($q_owner->created_at)->diffForHumans() }}{{-- クエリビルだで取得したタイムスタンプはparseをかける必要がある
                    @endforeach --}}
                </div>
            </div>
        </div>
